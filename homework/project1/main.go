...
func conn() (engin *xorm.Engine, err error) {
	engine, err := xorm.NewEngine("mysql", "xxxxxx:xxxxxx@tcp(localhost:3306)/test?charset=utf8")
	if err != nil {
		return engin, err
	}
	return engine, nil
...
func other(engine *xorm.Engine) error {
	var ids []Instance
	err := engine.Table("data_map_instances").Where("InstanceId = 'aaaaa'").Find(&ids)
	if err != nil {
		return err
	}
	return nil
...
func query(engine *xorm.Engine) error {
	var name string
	err := engine.DB().QueryRow("select InstanceId, Regiond, Status from data_map_instances where InstanceId = ?", "aaaaa").Scan(&name)
	if err != nil {
